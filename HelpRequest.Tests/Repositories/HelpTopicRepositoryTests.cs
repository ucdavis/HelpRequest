...
namespace HelpRequest.Tests.Repositories
{
	...
	public class HelpTopicRepositoryTests : AbstractRepositoryTests<HelpTopic, int, HelpTopicMap>
	{
		...
		[TestMethod]
		public void TestCanCorrectlyMapHelpTopic()
		{
			#region Arrange
			var id = HelpTopicRepository.Queryable.Max(x => x.Id) + 1;
			var session = NHibernateSessionManager.Instance.GetSession();
			#endregion Arrange

			#region Act/Assert
			new PersistenceSpecification<HelpTopic>(session)
				.CheckProperty(c => c.Id, id)
				.CheckProperty(c => c.Answer, "Answer")
				.CheckProperty(c => c.AppFilter, "HelpRequest")
				.CheckProperty(c => c.AvailableToPublic, true)
				.CheckProperty(c => c.IsActive, true)
				.CheckProperty(c => c.IsVideo, false)
				.CheckProperty(c => c.NumberOfReads, 100)
				.CheckProperty(c => c.Question, "Question?")
				.CheckProperty(c => c.VideoName, "Video Name")
				.VerifyTheMappings();
		...
		public void TestMapHelpTopicFails1()
		{
			#region Arrange
			var id = HelpTopicRepository.Queryable.Max(x => x.Id) + 1;
			var session = NHibernateSessionManager.Instance.GetSession();
			...
			try
			{
				...
				new PersistenceSpecification<HelpTopic>(session)
					.CheckProperty(c => c.Id, id)
					.CheckProperty(c => c.Answer, 23) //This is a string
					.CheckProperty(c => c.AppFilter, "HelpRequest")
					.CheckProperty(c => c.AvailableToPublic, true)
					.CheckProperty(c => c.IsActive, true)
					.CheckProperty(c => c.IsVideo, false)
			...
			catch (Exception ex)
			{
				#region Assert
				Assert.IsNotNull(ex);
				Assert.AreEqual("Error while trying to set property Answer", ex.Message);
				#endregion Assert
				throw;
			}
		}
		...
		[TestMethod]
		[ExpectedException(typeof(ApplicationException))]
		public void TestMapHelpTopicFails2()
		{
			#region Arrange
			var id = HelpTopicRepository.Queryable.Max(x => x.Id) + 2; //This is wrong, the next identity value would be +1 not +2
			var session = NHibernateSessionManager.Instance.GetSession();
			#endregion Arrange
			...
			catch (Exception ex)
			{
				#region Assert
				Assert.IsNotNull(ex);
				Assert.AreEqual("For property 'Id' expected '7' of type 'System.Int32' but got '6' of type 'System.Int32'", ex.Message);
				#endregion Assert
				throw;
			}
		}
